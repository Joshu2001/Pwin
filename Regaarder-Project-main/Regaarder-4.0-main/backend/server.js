...
const buildShareHtml = ({ title, description, image, url, redirectUrl, type = 'website' }, req) => {
  ...
  // Always show an image — use backend's own origin so WhatsApp/social crawlers can reach it
  const backendOrigin = PUBLIC_BACKEND_URL || (req ? `${req.protocol}://${req.get('host')}` : WEB_URL);
  const safeImage = escapeHtml(image || `${backendOrigin}/regaarder-og-banner.png`);
  return `<!doctype html>
<html lang="en">
  <head>
    <meta charset="utf-8" />
  ...
  <body style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; background: #0b0b0b; color: #fff; margin: 0; padding: 0;">
    <main style="max-width: 680px; margin: 40px auto; padding: 20px; text-align: center;">
      <img src="${safeImage}" alt="${safeTitle}" style="max-width: 100%; border-radius: 12px; margin-bottom: 16px;" />
      <h1 style="font-size: 22px; margin: 0 0 12px;">${safeTitle}</h1>
      <p style="font-size: 14px; opacity: 0.8;">${safeDesc}</p>
      <p style="font-size: 12px; opacity: 0.7; margin-top: 16px;">Opening Regaarder… If nothing happens, <a href="${safeRedirect}" style="color: #CB8A00;">tap here</a>.</p>
    </main>
  </body>
</html>`;
};
  ...
  try {
    ...
    videos.forEach((v) => {
      if (videoIds.includes(v.id)) {
        ...
        if (!Array.isArray(v.ads.overlays)) {
          v.ads.overlays = [];
        }
        if (!Array.isArray(v.ads.bottom)) {
          v.ads.bottom = v.ads.bottom || [];
        }
        v.ads.overlays.push({
...
// Serve OG branding banner for social share previews
app.get('/regaarder-og-banner.png', (req, res) => {
  const bannerPath = path.join(__dirname, 'regaarder-og-banner.png');
...
app.get('/share/video/:id', (req, res) => {
  try {
    ...
    return res.status(200).send(buildShareHtml({ title, description, image, url, redirectUrl, type: 'video.other' }, req));
  } catch (e) {
    res.set('Content-Type', 'text/html');
...
app.get('/share/request/:id', (req, res) => {
  try {
    ...
    const requesterName = request?.requesterName || (request?.creator && request.creator.name) || request?.company || '';
    const title = request?.title ? `${request.title} — Regaarder` : 'View this request on Regaarder';
    const rawDesc = request?.description || '';
    const description = rawDesc
      ? `${rawDesc.slice(0, 150)}${rawDesc.length > 150 ? '…' : ''}${requesterName ? ` — requested by ${requesterName}` : ''}`
      : `Support this request on Regaarder${requesterName ? ` by ${requesterName}` : ''}`;
    ...
    res.set('Content-Type', 'text/html');
    return res.status(200).send(buildShareHtml({ title, description, image, url, redirectUrl, type: 'website' }, req));
  } catch (e) {
    res.set('Content-Type', 'text/html');
    return res.status(200).send(buildShareHtml({
      title: 'Regaarder',
...
app.get('/share/profile/:key', (req, res) => {
  try {
    ...
    res.set('Content-Type', 'text/html');
    return res.status(200).send(buildShareHtml({ title, description, image, url, redirectUrl, type: 'profile' }, req));
  } catch (e) {
    res.set('Content-Type', 'text/html');
    return res.status(200).send(buildShareHtml({
      title: 'Regaarder',
...
// ── TEMPORARY admin cleanup: keep only specified video IDs, delete the rest + their files ──
app.post('/admin/cleanup-videos', (req, res) => {
  try {
    const secret = req.headers['x-cleanup-secret'];
    if (secret !== 'regaarder-cleanup-2026') return res.status(403).json({ error: 'Forbidden' });

    const { keepIds } = req.body; // array of video ID strings to keep
    if (!Array.isArray(keepIds)) return res.status(400).json({ error: 'keepIds must be an array' });
    ...
    // Extract upload filenames from a video's URLs
    const extractFilenames = (video) => {
      const names = [];
      const urlFields = ['imageUrl', 'videoUrl', 'thumbnailUrl', 'thumbUrl', 'poster', 'cover'];
      urlFields.forEach(field => {
        const val = video[field];
        if (val && typeof val === 'string' && val.includes('/uploads/')) {
          const parts = val.split('/uploads/');
          if (parts[1]) names.push(parts[1].split('?')[0]);
        }
      });
      return names;
    };

    // Collect filenames referenced by KEPT videos (don't delete these)
    const keepFilenames = new Set();
    videos.forEach(v => {
      if (keepSet.has(String(v.id))) {
        extractFilenames(v).forEach(f => keepFilenames.add(f));
        kept.push(v);
      } else {
        deletedRecords.push({ id: v.id, title: v.title });
        // Delete files for this video
        extractFilenames(v).forEach(filename => {
          if (keepFilenames.has(filename)) return; // shared file, don't delete
          const filePath = path.join(UPLOAD_DIR, filename);
          try {
            if (fs.existsSync(filePath)) {
              const stat = fs.statSync(filePath);
              fs.unlinkSync(filePath);
              deletedFiles.push({ file: filename, sizeKB: Math.round(stat.size / 1024) });
            }
          } catch (e) {
    ...
    // Also clean orphan files in uploads/ that no kept video references
    let orphanFiles = [];
    try {
      const allFiles = fs.readdirSync(UPLOAD_DIR);
      allFiles.forEach(f => {
        if (!keepFilenames.has(f)) {
          const filePath = path.join(UPLOAD_DIR, f);
          try {
            const stat = fs.statSync(filePath);
            if (stat.isFile()) {
              fs.unlinkSync(filePath);
              orphanFiles.push({ file: f, sizeKB: Math.round(stat.size / 1024) });
            }
          } catch (e) {
            failedFiles.push({ file: f, error: e.message });
          }
        }
      });
    } catch (e) {
    ...
    // Save only kept videos
    writeVideos(kept);

    const totalFreedKB = [...deletedFiles, ...orphanFiles].reduce((sum, f) => sum + (f.sizeKB || 0), 0);

    return res.json({
      success: true,
      keptVideos: kept.length,
      deletedRecords: deletedRecords.length,
      deletedFiles: deletedFiles.length,
      orphanFilesDeleted: orphanFiles.length,
      totalFreedMB: Math.round(totalFreedKB / 1024),
      failedFiles,
      deletedRecordTitles: deletedRecords.map(r => r.title),
    });
  } catch (err) {
